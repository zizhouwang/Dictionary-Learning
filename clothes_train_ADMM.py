...
for a2 in range(1):
    ...
    the_ADMM_lambda=np.zeros(Ds.shape[2]*Ds.shape[0])
    the_ADMM_H=np.zeros((Ds.shape[2]*Ds.shape[0],Ds.shape[2]*Ds.shape[0]))
    D_all=Ds_to_D_all(Ds)
    gram=D_all.T@D_all
    p1=1.5
    p2=1.5
    c1=1
    ...
    for i in range(update_times):
        ...
        for class_index in range(n_classes):
            ...
            D_all = Ds_to_D_all(Ds)
            gram_diag=np.diag(D_all.T@D_all)
            the_ADMM_lambda+=c1*(gram_diag-np.diag(np.eye(gram_diag.shape[0])))
            the_ADMM_H+=c2*(gram-D_all.T@D_all)
            c1=c1*p1
            c2=c2*p2
    end_t=time.time()
