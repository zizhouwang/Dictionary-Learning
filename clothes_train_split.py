...
for a2 in range(1):
    ...
    train_data_reg=data['train_data']
    train_Annotation=data['train_Annotation']
    separated_dimen=64
    separated_times=train_data_reg.shape[0]//separated_dimen+1
    train_data_reg_separated=np.zeros((separated_dimen,separated_times*train_data_reg.shape[1]))
    train_Annotation_separated=np.empty((train_Annotation.shape[0],separated_times*train_data_reg.shape[1]))
    for i in range(train_data_reg.shape[1]):
        for j in range(separated_times):
            one_separated=train_data_reg[:,i][j*separated_dimen:(j+1)*separated_dimen]
            train_data_reg_separated[:one_separated.shape[0],i*separated_times+j]=one_separated
            train_Annotation_separated[:,i*separated_times+j]=train_Annotation[:,i]
    train_Annotation= train_Annotation * 2 - 1
    change_num=1e-7
    train_data_reg=preprocessing.normalize(train_data_reg.T, norm='l2').T
    train_data_reg=norm_Ys(train_data_reg)
    train_data_reg,train_Annotation=Separate_data(train_data_reg,train_Annotation,0.2)
    labels_index=np.empty((train_Annotation.shape[0], train_Annotation.shape[1]),dtype=int)
    labels_index[:]=-1
    images_count=np.empty((5),dtype=int)
    for i in range(train_Annotation.shape[0]):
        one_label_mat=train_Annotation[i]
    ...
    train_number=300
    update_times=8000
    im_vec_len=train_data_reg.shape[0]
    ...
    n_features = train_data_reg.shape[0]
    ...
    # if os.path.isfile(inds_of_file_path_path) and 5==2:
    ...
    #             pdb.set_trace()
    #         inds_of_file_path[class_index][:images_count[class_index]]=labels_of_one_class
    inds_of_file_path=labels_index
    ...
    for class_index in range(n_classes):
        D[:,:start_init_number] = train_data_reg[:, inds_of_file_path[class_index][:start_init_number]]
    ...
    for i in range(update_times):
        ...
        for class_index in range(n_classes):
            ...
            if i==0:
                ...
                Y_init= train_data_reg[:, img_init_indexs]
                ...
                the_H= train_Annotation[:, img_init_indexs]
            ...
            label_indexs_for_update=np.where(train_Annotation[class_index]==1)[0]
            new_index=[label_indexs_for_update[(i+start_init_number)%images_count[class_index]]]
            im_vec= train_data_reg[:, new_index]
            ...
            new_h= train_Annotation[:, new_index]
    ...
    D_all=preprocessing.normalize(D_all.T, norm='l2').T
    coder = SparseCoder(dictionary=D_all.T,transform_n_nonzero_coefs=transform_n_nonzero_coefs, transform_algorithm="omp")
    the_X=(coder.transform(train_data_reg.T)).T
    # D_all=incoherent(D_all,image_vecs,the_X,1)
    D_all=preprocessing.normalize(D_all.T, norm='l2').T
